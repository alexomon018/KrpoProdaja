...
describe('Avatar', () => {
  it('renders with image source', () => {
    const { container } = render(<Avatar src="/test.jpg" alt="Test User" fallback="TU" />);
  ...
  it('applies correct size classes', () => {
    const { rerender, container } = render(<Avatar alt="Test" size="sm" fallback="T" />);
    expect(screen.getByText('T')).toBeInTheDocument();
    expect(container.querySelector('.h-8.w-8')).toBeInTheDocument();

    rerender(<Avatar alt="Test" size="lg" fallback="T" />);
    expect(screen.getByText('T')).toBeInTheDocument();
    expect(container.querySelector('.h-12.w-12')).toBeInTheDocument();
  });

  it('shows fallback when image fails to load', () => {
    render(<Avatar src="/invalid.jpg" alt="Test User" fallback="TU" />);
