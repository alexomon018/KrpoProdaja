...
```
Registration Flow:
/auth/register → /auth/verify-email → / (home)
                    ↓ (with token)
                /auth/verify-email?token=xxx

Login Flow:
/auth/login → / (home or previous page)

Password Reset Flow:
/auth/reset-password → Email → /auth/reset-password/[token] → /auth/login
```
...
1. Run `npm run dev`
2. Navigate to `/auth/login`, `/auth/register`, etc.
